<template>
    ...
    <div><h1>Modificar pedido</h1></div>
          ...
          <div class="form-row">
            <div class="d-flex flex-row">
              <div>
                ...
                <select class="form-select" v-model="pedido.idCliente" required>
              ...
              <div>
                ...
                <input type="date" class="form-control" id="fecha" v-model="pedido.fecha" required>
              </div>
            </div>
  ...
  export default {
    ...
    created() {
      this.pedido.idPedido = this.$route.params.idPedido;
      this.obtenerPedido();
    },
    methods: {
      async obtenerPedido() {
        try {
          ...
          if (response.ok){
            const data = await response.json();
              ...
              fecha: data.fecha.split('T')[0], // Formatear la fecha para el input de tipo date
              idCliente: data.idCliente,
              nombreCliente: data.nombreCliente,
              idEmpleado: data.vendedor,
              nombreEmpleado: data.nombreVendedor
            };
    ...
    async enviarFormulario() {
      try {
        const formData = {
          ...
          const response = await fetch('http://localhost:8088/pedido/modificar', {
            method: 'PUT',
            headers: {
          ...
          if (response.ok) {
            this.$router.push({ path: '/verpedidos', query: { mensaje: 'Pedido editado',  tipo: 'satisfactorio' } });
          } else {
            this.$router.push({path:'/verpedidos', query: { mensaje: 'Error al actualizar pedido', tipo:'error'}})
          }
        } catch (error) {
          console.error('Error:', error);
        }
